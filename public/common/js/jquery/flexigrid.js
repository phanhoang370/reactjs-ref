...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		if (p.colModel) { //create model if any
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					...
					// Patch 2015-07-31
					th.halign = cm.headerAlign? cm.headerAlign: p.headerAlign? p.headerAlign: 'center'; // PATCH SG#1
					if (cm.width) {
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			$(thdiv).css({
				textAlign: this.halign, // 2015-07-31 
				width: this.width + 'px'
			});
		...
		if (p.title) {
			...
			g.mDiv.style.textAlign = p.titleAlign? p.titleAlign: 'left'; // PATCH SG#2	2015-07-31
